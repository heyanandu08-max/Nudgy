...
/** A small, polite toast in the bottom-right corner offering a one-minute review. */
export function ReviewNudge({ nudge, onClose }: { nudge: Nudge; onClose: () => void }) {
  ...
  return (
    <div ref={ref} className="review-nudge" role="dialog" aria-label={t("review.nudgeTitle")}>
      <p className="review-nudge__title">{t("review.nudgeTitle")}</p>
      <p>{t("review.nudgeBody", { skill: nudge.skill_name.toLowerCase() })}</p>
      {nudge.due_count > 1 && <p className="review-nudge__more">{t("review.nudgeMore", { count: nudge.due_count })}</p>}
      <div className="review-nudge__actions">
