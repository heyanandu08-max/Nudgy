...
class Plan:
    ...
    # billing interval ("month" | "year") → env var holding the PayPal plan ID
    price_envs: dict[str, str] = field(default_factory=dict)
    # billing interval → price shown in the app, e.g. "$20"
    labels: dict[str, str] = field(default_factory=dict)
    per_seat: bool = False

    def billing_plan_id(self, interval: str = "month") -> str | None:
    ...
    def offer(self) -> dict[str, str]:
        ...
        return {i: self.labels.get(i, "") for i in self.price_envs if self.billing_plan_id(i)}
...
def plan_for_price(price_id: str | None) -> str | None:
    for p in plans().values():
        if price_id and price_id in (p.billing_plan_id(i) for i in p.price_envs):
            return p.id
    return None
