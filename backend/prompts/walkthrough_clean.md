...
- `instruction`: plain, direct spoken sentence in {{language}} (no exclamation marks) telling the learner what to do. Say shortcuts in words ("press Control plus S"). If the author typed specific text that the learner should type too, include it; if it looks personal (names, emails, numbers that are clearly private) say "type your …" instead. Never include "[hidden]" content.
